...
class TestRoiAlign(TorchBaseTest):
    ...
    class RoiAlignDetectionHead(torch.nn.Module):
        def __init__(self):
            super().__init__()
    ...
    def test_roi_align_detection_head_smoke(self):
        input_data = [
            torch.randn(2, 3, 8, 9),
            torch.tensor(
                [
        ...
        frontends_to_test = [TorchFrontend.TORCHSCRIPT]
        if _HAS_TORCH_EXPORT_API:
            frontends_to_test.append(TorchFrontend.TORCHEXPORT)

        for frontend in frontends_to_test:
            prog = self._convert_to_milinternal(
                self.RoiAlignDetectionHead().eval(),
                input_data,
                frontend,
            )
            ops = get_op_types_in_program(prog)
